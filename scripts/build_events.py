...
def norm(s):
    s=str(s).lower()
    table=str.maketrans("éèêëàâäîïôöùûüçñ","eeeeaaaiioouucn")
    s=s.translate(table)
...
def parse_date(v):
    ...
    if isinstance(v,dict):
        # Tourinsoft/OpenDataSoft date objects
...
def find_date(o):
    preferred=("datedebut","datedebutmanifestation","datededebut","startdate","debut","firstdate")
    vals=[]
    for d in walk(o):
        if isinstance(d,dict):
            for k,v in d.items():
                if any(x in norm(k) for x in preferred): vals.append(v)
    for v in vals:
        d=parse_date(v)
        if d:return d
    return None
...
def coords(o):
    for d in walk(o):
        ...
        for k,v in d.items():
            nk=norm(k)
            if isinstance(v,(list,tuple)) and len(v)>=2 and any(x in nk for x in ("geo","coord","point")):
                try:
                    a,b=float(str(v[0]).replace(",",".")),float(str(v[1]).replace(",","."))
                    if 40<a<52 and -6<b<10:return a,b
...
# IMPORTANT: accept all common OpenDataSoft export envelopes.
...
for o in records:
    ...
    outdoor=bool(re.search(r"plein.?air|extérieur|exterieur|stade|terrain|parc|marché|marche|vide.?grenier|brocante|fête|fete|festival|concert|course|randonn|sport|moto|auto|rassemblement|braderie|kermesse|feu d.?artifice",text))

    events.append({
        "id":eid,"date":d.isoformat(),"title":title,"place":place,
        "description":find_text(o,("description","descriptif","resume","detail","presentation")),
        "distance":round(distance,1),"outdoor":outdoor,
